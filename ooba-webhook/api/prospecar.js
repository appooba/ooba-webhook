...
// Recebe uma lista de leads e dispara o template aprovado automaticamente
// Body: { secret: "ooba2026", leads: [{ phone: "55...", nome: "..." }, ...] }
  ...
  for (const lead of leads) {
    ...
    try {
      ...
      if (existing.rows.length === 0) {
        await client.query(
          `INSERT INTO leads (phone, nome, first_message, etapa_funil, status, origem, prospeccao_data, updated_at) 
           VALUES ($1, $2, $3, 'abertura', 'novo', 'prospeccao', NOW(), NOW())`,
          [phone, nome, `Prospecção automática: ${nome}`]
        );
      } else {
        // Lead já existe — pular se já foi contatado
      ...
      // 2. Disparar template via WhatsApp API
      ...
      // Rate limit: 2 segundos entre envios
